...
public:
    ...
    /*when we talk about move operation, we always talk about rvalues
    */
    DynamicArray(DynamicArray &&other) noexcept; //&& - rvalue referecne
...
DynamicArray<T>::DynamicArray(DynamicArray &&other) noexcept
    : m_length(std::move(other.m_length)), m_capacity(std::move(other.m_capacity)),
      ...
      // with move,the memory is swiped

{
    other.m_length = 0;
    other.m_capacity = 0;
    other.m_data = nullptr; // now the other.m_data cannot point to the memory as the our this->m_data is pointing now to the same memory
