...
int main()
{
    ...
    my_vector.erase(remove_it1, my_vector.end()); // remove_it1 is an iterator to the -1 value
    print_vector(my_vector);

    auto remove_it2 = std::remove_if(my_vector.begin(), my_vector.end(), is_negative);
    my_vector.erase(remove_it2, my_vector.end()); // remove_it2 is an iterator to the shows negative values
    print_vector(my_vector);
