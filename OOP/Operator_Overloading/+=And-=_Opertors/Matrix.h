...
Matrix<T>& Matrix<T>::operator-=(const Matrix<T> &rhs){
    ...
    return *this; // returns the object- return type is a reference. so an alias of the object is returned(means the ojbect itself)
    // Jan explanation: the += operator should return reference
